...
def team_delete(request, pk):
   try:
       ...
       return redirect('team_list')
   if team.drivers.filter(registered_races__isnull=False).exists():
...
def race_create(request):
    ...
    return render(request, 'races/race_form.html',{'form':form})
...
def race_delete(request, pk):
   try:
       ...
       return redirect('race_list')
   if race.drivers.filter(registered_races__isnull=False).exists():
...
class RaceUpdate(generics.RetrieveUpdateAPIView):
    queryset = Race.objects.all()
    serializer_class = raceSerializer
